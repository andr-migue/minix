...
\hline
\multicolumn{4}{|l|}{\textit{Al detener el proceso CPU-bound:}} \\
\hline
20--25 s & 9 $\to$ 8 (recuperaci\'on) & 7 (estable) & \texttt{quantum\_count = 0}: recupera un nivel \\
\hline
25--30 s & 8 $\to$ 7 (base) & 7 (estable) & Recuperaci\'on completa tras dos ventanas \\
\hline
\end{longtable}

El proceso CPU-bound mostr\'o la degradaci\'on esperada en cada ventana de balance (un nivel por ventana hasta el techo), mientras que el proceso interactivo mantuvo su prioridad base durante toda la prueba. La recuperaci\'on gradual funcion\'o correctamente: tras dejar de agotar quantums, el proceso recuper\'o un nivel de prioridad por cada ventana en que \texttt{quantum\_count} result\'o cero. Esto confirma que la pol\'itica implementada distingue correctamente entre ambos perfiles de ejecuci\'on y respeta los l\'imites definidos por \texttt{MIN\_USER\_Q} y \texttt{MAX\_PENALTY\_LEVEL}.
